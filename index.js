...
function callback(error, response, body, cb) {
  ...
  if (isJson(body)) {
    global.bodyJson = JSON.parse(body);
    cb(null, global.bodyJson); //instead of sending data directly to view, send it to async callback to merge it latter
  }
}
app.get("/check", async (req, res) => {
  // res.render("views/trangchu");
  if (global.bodyJson) {
    console.log("bodyjson", global.bodyJson);
  ...
  request.post(
    {
      ...
      body: `
    [{"pageSize":1000,"pageNumber":"0","query":[{"index":"es-contractor-selection",
    ...
    function (error, response, body) {
      if (body) {
        ...
        if (
          global.bodyJson &&
          global.bodyJson.page &&
          global.bodyJson.page.content
        ) {
          global.bodyJson.page.content.map((item, index) => {
